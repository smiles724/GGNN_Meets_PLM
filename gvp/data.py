...
def _normalize(tensor, dim=-1):
    ...
    """
    return torch.nan_to_num(torch.div(tensor, torch.norm(tensor, dim=dim, keepdim=True)))   # nan_to_num is only available >= 1.8
...
def _rbf(D, D_min=0., D_max=20., D_count=16, device='cpu'):
    ...
    That is, if `D` has shape [...dims], then the returned pe_id will have
    shape [...dims, D_count].
    """
...
class ProteinGraphDataset(data.Dataset):
    ...
    -seq        sequence converted to int pe_id according to `self.letter_to_num`, shape [n_nodes]
    ...
    def __init__(self, data_list, num_positional_embeddings=16, top_k=30, num_rbf=16, device="cpu"):
        ...
        self.letter_to_num = {'C': 4, 'D': 3, 'S': 15, 'Q': 5, 'K': 11, 'I': 9, 'P': 14, 'T': 16, 'F': 13, 'A': 0, 'G': 7, 'H': 8, 'E': 6, 'L': 10, 'R': 1, 'W': 17, 'V': 19,
                              'N': 2, 'Y': 18, 'M': 12}
    ...
    def _featurize_as_graph(self, protein):
        ...
        with torch.no_grad():
            ...
            edge_index = torch_cluster.knn_graph(X_ca, k=self.top_k)    # KNN建图，获取edge index (2, n_e)

            pos_embeddings = self._positional_embeddings(edge_index)    # 计算relative PE
            E_vectors = X_ca[edge_index[0]] - X_ca[edge_index[1]]    # 计算CA间的空间距离
            rbf = _rbf(E_vectors.norm(dim=-1), D_count=self.num_rbf, device=self.device)   # 将距离转化为RBF表征

            dihedrals = self._dihedrals(coords)     # 计算二面角
            orientations = self._orientations(X_ca)     # 计算方向向量
            sidechains = self._sidechains(coords)   # 用生物学公式计算侧链位置

            node_s = dihedrals    # node的scalar feature (invariant)
            node_v = torch.cat([orientations, sidechains.unsqueeze(-2)], dim=-2)     # node的vector feature
            edge_s = torch.cat([rbf, pos_embeddings], dim=-1)   # edge的scalar feature
            edge_v = _normalize(E_vectors).unsqueeze(-2)     # edge的vector feature

            node_s, node_v, edge_s, edge_v = map(torch.nan_to_num, (node_s, node_v, edge_s, edge_v))

        data = torch_geometric.data.Data(x=X_ca, seq=seq, name=name, node_s=node_s, node_v=node_v, edge_s=edge_s, edge_v=edge_v, edge_index=edge_index, mask=mask)
        return data

    def _positional_embeddings(self, edge_index):
        # From https://github.com/jingraham/neurips19-graph-protein-design
        num_embeddings = self.num_positional_embeddings
        d = edge_index[0] - edge_index[1]   # 计算两个氨基酸在序列上的相对距离，有方向

        frequency = torch.exp(torch.arange(0, num_embeddings, 2, dtype=torch.float32, device=self.device) * -(np.log(10000.0) / num_embeddings))
    ...
    def _orientations(self, X):
        forward = _normalize(X[1:] - X[:-1])    # 从前往后的方向向量
        backward = _normalize(X[:-1] - X[1:])    # 从后往前的方向向量
